  ...
  return (
    <section id="skills" className="relative mx-auto max-w-6xl px-4 pb-12 sm:px-8 lg:px-12" data-reveal>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:items-end">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.35em] text-[color:var(--accent)]">Skills</p>
      ...
      <div className="mt-7 grid gap-4 lg:grid-cols-3">
        {skillGroups.map((group, index) => (
          <article
            key={group.title}
            className="glass-card group rounded-[26px] p-5 transition-all duration-300 hover:-translate-y-1"
            data-reveal
          >
            ...
            <div className="mt-3 grid gap-2 text-[14px] text-[color:var(--ink)]">
              {group.items.map((item) => (
                <div key={`${group.title}-${item}`} className="rounded-xl border border-white/70 bg-white/80 px-3 py-2">
                  {item}
                </div>
              ))}
            </div>
            <div className="mt-4 h-1 w-full overflow-hidden rounded-full bg-[color:var(--paper-strong)]">
              <div className="h-full rounded-full bg-[color:var(--accent-2)]" style={{ width: `${76 + index * 8}%` }} />
            </div>
          </article>
        ))}
      </div>
